...
OUT_BASE_DIR = r"E:\S26 Info Design\Full Runs 3-11"
...
# Experiment settings
# ==========================
PATH_BUILDER = "maxflow"      # maxflow" or "k_shortest"
K_PATHS = 20                  # only used when PATH_BUILDER == "k_shortest"
MAX_DECOMP_PATHS = None
FLOW_EPS = 1e-9
pckl = False # whether to save large reference points as pickle (True) or JSON (False, but may lose precision and be very large files)  
save_full =  False


# Behavioral parameters (edit if desired)
MARKET_PENETRATION_RANGE = np.linspace(0.0, 1.0, num=11)
CREDIBILITY_RANGE = np.linspace(0.0, 1.0, num=11)
...
def build_graph_from_sioux_falls_csv(net_df: pd.DataFrame, node_df: pd.DataFrame) -> nx.DiGraph:
    ...
    We map exactly into a BPR form while deriving a meaningful capacity from a4:
        t(x) = t0 * (1 + alpha * (x/c)^beta)
    with beta fixed at 4 and alpha fixed at 0.15. Solving for c to keep
    t(x)=a0 + a4 x^4 gives: c = ((alpha * a0) / a4)^(1/4) when a4>0.
    """
    G = nx.DiGraph()
    ...
    for _, r in net_df.iterrows():
        ...
        t0 = a0
        if a4 > 0 and a0 > 0:
            cap = ((alpha_fixed * a0) / a4) ** 0.25  # preserves a0 + a4 x^4 with beta=4
            alpha = alpha_fixed
        else:
            cap = 1.0
            alpha = 0.0
            print(f"Warning: link ({u}->{v}) has non-positive a4={a4} or a0={a0}. Setting capacity to 1.0 and alpha to 0.0 (free flow time only).")
...
def export_path_table(net: Dict[str, Any], od_demands: Dict[ODPair, float], results: Dict[str, Any], out_csv: str) -> None:
    ...
    for od in net["ods"]:
        ...
        for k in range(K):
            ...
            caps = net["per_od"][od].get("path_flow_caps")
            if caps is not None and k < len(caps):
                rec["path_basis_flow"] = float(caps[k])
...
# ==========================
def main() -> None:
    for MARKET_PENETRATION in MARKET_PENETRATION_RANGE:
        for CREDIBILITY in CREDIBILITY_RANGE:
            print(f"Running with market_penetration={MARKET_PENETRATION:.2f}, credibility={CREDIBILITY:.2f}...")
            
            ts = datetime.now().strftime("%Y%m%d_%H%M")
            out_root = os.path.join(OUT_BASE_DIR, f"SiouxFalls_full_run_{ts}_pen{MARKET_PENETRATION}_cred{CREDIBILITY}")
            ...
            if PATH_BUILDER == "k_shortest":
                net = hs.build_multi_od_network_data(G, od_pairs=od_pairs, K=K_PATHS, path_builder="k_shortest")
            else:  # maxflow decomposition
                net = hs.build_multi_od_network_data(
                    G,
                    od_pairs=od_pairs,
                    K=None,
                    path_builder="maxflow",
            ...
            models = [
                ...
                ("ARA_outcome",             dict(risk_model="ara",      utility_base="outcome",  reference_override=None))
