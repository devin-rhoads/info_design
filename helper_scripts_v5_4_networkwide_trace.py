...
def build_multi_od_network_data(
    ...
    """
    Build per-OD K-path subnetworks AND a global (union) link index so we can compute
    congestion jointly across all OD pairs.

    Returns dict with:
      - 'ods': list[(o,d)]
      - 'per_od': dict[(o,d)] -> {'paths','K','links_local','A_local','A_global'}
      - 'global_links': list[(u,v)]
      - 'bpr_arrays': arrays aligned with global_links
    """
    if not isinstance(G, nx.DiGraph):
    ...
    for od in od_list:
        o, d = od
        net = build_network_data(G, s_node=o, t_node=d, K=K)
        if net is None:
    ...
    # Global link order: preserve original graph insertion order where possible
    global_links = [e for e in G.edges() if e in used_links]
    # Safety: include any missing used links
    for e in used_links:
    ...
    # Build global BPR arrays (aligned with global_links)
